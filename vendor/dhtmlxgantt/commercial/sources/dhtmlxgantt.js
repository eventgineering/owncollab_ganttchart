...
gantt._need_dynamic_loading = function needLoading(id){
	if(gantt.config.branch_loading && gantt._load_url){
		...
		if (!was_rendered && (gantt._has_children(id) && !gantt.getChildren())) {
			return true;
		}
	}
	return false;
...
gantt._scale_helpers = {
	...
	_ignore_time_config : function(date, unit){
		if(this.config.skip_off_time){
			return !this.isWorkTime(date, unit || undefined);
		}
		return false;
	},
...
gantt._scale_helpers.processIgnores = function(config){
	...
	if(gantt.ignore_time || gantt.config.skip_off_time){
		...
		for(var i=0; i < config.trace_x.length; i++){
			...
			if(ignore.call(gantt, config.trace_x[i]) || this._ignore_time_config.call(gantt,config.trace_x[i], scaleUnit)){
...
gantt._working_time_helper = {
	...
	get_calendar: function(){
		...
		if(!gantt.config.worktimes.global){
...
gantt.config.worktimes = {
	global: gantt._working_time_helper.default_calendar()
};
    ...
    if(top*1 == top){
		...
		if(gantt.config.smart_rendering){
			if(this.$grid_data.scrollTop != top){
				this.$grid_data.scrollTop = this.$task_data.scrollTop % gantt.config.row_height;
			}
		}
